defmodule CheckEscript.Tasks do
      ...
      for partition <- 1..max(partitions, 0)//1, into: %{} do
        ...
        task_def =
          if mock_mode do
            ...
            resolved_test_flags = String.replace(test_flags, "{partition}", to_string(partition))

            {task_name, "sh",
             [
               "-c",
               "ELIXIR_ERL_OPTIONS='+S #{test_procs}:#{test_procs}' MIX_TEST_PARTITION=#{partition} mix #{test_runner} #{test_path} #{resolved_test_flags}#{coverage_flags} --partitions #{partitions}"
             ], partition, partitions}
          end
  ...
  def test_runner_cmd(:native, _partition), do: {"test", " --cover"}
  def test_runner_cmd(:coveralls, _partition), do: {"coveralls", ""}
  def test_runner_cmd(_, _partition), do: {"test", ""}
